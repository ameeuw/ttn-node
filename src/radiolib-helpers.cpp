    ...
    while (true)
    {
        ...
        if (joined)
        {
            ...
            if (xQueueReceive(uplinkQueue, &(prxuplinkMessage), portMAX_DELAY))
            {
                ...
                if (joined)
                {
                    int16_t result = scheduleUplink(prxuplinkMessage->fport, prxuplinkMessage->data, prxuplinkMessage->length, false);
                    ...
                    {
                        Serial.printf("Uplink not scheduled, error code: %d\n", result);
                    }
                    count = 0;
                }
